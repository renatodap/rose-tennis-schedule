# Plan 03-03: Create Date Detail Page

## Objective
Build the detail page showing all attendees for a specific practice date.

## Context
@.planning/BRIEF.md
@components/attendance/AttendanceList.tsx
@components/attendance/ConfirmButton.tsx
@lib/hooks/useAttendance.ts

## Tasks

### Task 1: Create date detail page
**Type:** create
**Files:** `app/(dashboard)/practice/[date]/page.tsx`
**Action:** Create detail page:
```typescript
'use client';
...
import { PRACTICE_TIME, PLAYER_LOCATIONS } from '@/lib/constants';
...
```
**Verify:** Page renders with attendance details

### Task 2: Create page folder structure
**Type:** create
**Files:** `app/(dashboard)/practice/[date]/` directory
**Action:** Ensure folder structure exists for dynamic route
**Verify:** Route works when navigating

## Verification
```bash
npm run build
npm run dev
# Navigate to /practice/2024-12-08 and verify
```

## Success Criteria
- [ ] Detail page shows date info
- [ ] Shows list of confirmed/declined users
- [ ] Confirm/decline buttons work
- [ ] Back navigation works
- [ ] Past dates show as read-only
