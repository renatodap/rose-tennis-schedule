...
/**
 * Multi-step dialog for editing existing forms
 * Steps: Basic Info -> Questions -> Target Audience
 */

import { useState, useEffect } from 'react';
import { FormQuestion, Form } from '@/lib/types/database.types';
...
import { isoToDateTimeLocal, dateTimeLocalToISO } from '@/lib/utils/time';

interface EditFormDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  form: Form | null;
  onSubmit: (formId: string, updates: Partial<Form>) => Promise<void>;
}

export function EditFormDialog({ open, onOpenChange, form, onSubmit }: EditFormDialogProps) {
  ...
  useEffect(() => {
    if (form && open) {
      ...
      setDueDate(form.due_date ? isoToDateTimeLocal(form.due_date) : '');
      setQuestions(form.questions);
      setGender(form.gender || 'all');
      setTeamLevel(form.team_level || 'all');
      setStep(1);
    }
  }, [form, open]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await onSubmit(form.id, {
        ...
        team_level: teamLevel === 'all' ? undefined : (teamLevel as TeamLevel),
      });
      resetForm();
      onOpenChange(false);
    } catch (error) {
      ...
      case 3:
        return (
          <div className="space-y-4">
            ...
            <div>
              <Label htmlFor="edit-team_level">Target Team Level</Label>
              <Select value={teamLevel} onValueChange={(val) => setTeamLevel(val as TeamLevel | 'all')}>
                <SelectTrigger id="edit-team_level">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
    ...
    <Dialog open={open} onOpenChange={(isOpen) => {
      ...
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Step {step} of 3:{' '}
            {step === 1 ? 'Basic Information' : step === 2 ? 'Edit Questions' : 'Target Audience'}
          </DialogDescription>
        </DialogHeader>
